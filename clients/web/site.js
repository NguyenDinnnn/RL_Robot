...
let allAlgoData = {};
let currentAlgoData = { rewards: [], totalReward: 0 }; // Biến mới để lưu trữ dữ liệu của thuật toán đang chạy
...
// Hàm move như trước (di chuyển robot 1 bước)
function move(dx, dy) {
...
// Auto step theo greedy (tham lam → chọn gần goal nhất)
...
// Chọn mode auto: "random" hoặc "greedy"
let autoMode = "random"; // mặc định random
...
async function runAlgorithm(algo){
    ...
    }, 200); // Tốc độ di chuyển: 200ms mỗi bước
}

// ---------------- Run A* (unchanged, vẫn hoạt động tốt) ----------------
async function runAStar(){
    ...
    try{
        ...
        const data = await resp.json();
        if(data.error){ resultDiv.innerHTML = `❌ Lỗi: ${data.error}`; return; }
